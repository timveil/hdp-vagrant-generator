...
# Vagrantfile generated by https://github.com/timveil/hdp-vagrant-generator on 6/6/16 1:45 PM
...
Vagrant.configure("2") do |config|
    ...
    if FFI::Platform::IS_MAC
        ...
        config.multihostsupdater.aliases = {'192.168.7.101' => ['example.hdp.local']}
    ...
    config.vm.hostname = 'example.hdp.local'
...
192.168.7.101   example.hdp.local
EOF
...
sed -i "s/^hostname=localhost/hostname=example.hdp.local/g" /etc/ambari-agent/conf/ambari-agent.ini
...
curl -s --show-error -H "X-Requested-By: ambari" -X POST -d '{"ViewInstanceInfo":{"instance_name":"FILES_VIEW_INSTANCE","label":"Files View","visible":true,"icon_path":"","icon64_path":"","properties":{"webhdfs.url":"webhdfs://example.hdp.local:50070","webhdfs.username":null,"webhdfs.auth":"auth=SIMPLE"},"description":"Files View installed by https://github.com/timveil/hdp-vagrant-veil.hdp.generator"}}' -u admin:admin http://example.hdp.local:8080/api/v1/views/FILES/versions/1.0.0/instances/Files
...
curl -s --show-error -H "X-Requested-By: ambari" -X POST -d '{"blueprint" : "example","default_password" : "password","host_groups" :[{"name" : "host_group_1","hosts" : [{"fqdn" : "example.hdp.local"}]}]}' -u admin:admin http://example.hdp.local:8080/api/v1/clusters/example
...
echo " "
echo "---------------------------------------------------------------------------------------------------------------"
echo "----- Cluster Install Complete!  http://example.hdp.local:8080"
echo "---------------------------------------------------------------------------------------------------------------"
echo " "
